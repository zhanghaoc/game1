...
template< typename T >
void read_chunk(std::fstream &from, std::string const &magic, std::vector< T > *to_) {
	assert(to_);
	auto &to = *to_;
...
template< typename T >
void write_chunk(std::string const &magic, std::vector< T > const &from, std::fstream *to_) {
	assert(magic.size() == 4);
	assert(to_);
	auto &to = *to_;
